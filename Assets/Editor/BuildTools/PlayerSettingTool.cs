...
public class PlayerSettingTool : EditorWindow
{
    ...
    private int _lastSelectGameTypeIndex = 1;
    ...
    private void OnGUI()
    {
        ...
        _scrollPos = EditorGUILayout.BeginScrollView(_scrollPos);
        {
            ...
            if (GUILayout.Button("保存配置(Resources/Setting和StreamingAssets)", GUILayout.Height(40)))
            {
                if (!CheckIsCompiling())
                {
                    SaveComeFromConfig();
    ...
    private void BuildPC(bool cmd = false)
    {
        ...
        string outputPath = string.Format("BuildWin/{0}/dhxx.exe", app_name);//BuildWin/bnz_1.5.6.0_empty/dhxx.exe
        string outputDir = Path.GetDirectoryName(outputPath);
        ...
        string res = BuildPipeline.BuildPlayer(FindEnabledEditorScenes(), outputPath, BuildTarget.StandaloneWindows64, buildoption);
        if (res.Length > 0)
        {
            throw new Exception("BuildPlayer failure: " + res);
        }
    ...
    {
        PlayerSettings.companyName = "BHT";
...
//        bundleIdentifier = string.Format("{0}.{1}.{2}", bundleIdentifier, _gameSettingData.domainType.ToLower(), _gameSettingData.updateMode.ToString().ToLower());
        ...
        PlayerSettings.applicationIdentifier = "com.bht.bnz";
    }
