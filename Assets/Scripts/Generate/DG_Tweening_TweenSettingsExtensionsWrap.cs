...
public class DG_Tweening_TweenSettingsExtensionsWrap
{
	...
	static int DoFloat(IntPtr L)
	{
		try
		{
			...
			var tween = DOTween.To(() => number, x => number = x, count, time)
				.SetEase(Ease.InSine)
				.OnUpdate(() => 
				{
					if(func != null)
	...
	static int DoValue(IntPtr L)
	{
		try
		{
			...
			float count = (float)LuaDLL.luaL_checknumber(L, 2);
			float time = (float)LuaDLL.luaL_checknumber(L, 3);
			LuaFunction updateFunc = ToLua.ToLuaFunction(L, 4);
			LuaFunction completeFunc = ToLua.ToLuaFunction(L, 5);
			var tween = DOTween.To(() => number, x => number = x, count, time)
				.SetEase(Ease.InSine)
				.OnUpdate(() => 
				{
				...
				.OnComplete(() =>
				{
					...
					{
						completeFunc.BeginPCall();
						//completeFunc.Push(number);
						completeFunc.PCall();
						completeFunc.EndPCall();
					}
				});
			//ToLua.PushObject(L, tween);
			return 1;
