...
function CLoginPhoneCtrl.ResetConnected(self)
	self.m_IsConnected = false
end
...
function CLoginPhoneCtrl.OnServerHello(self)
	...
	if(self.m_VerifyInfo.isLogin) then
		self:C2GSLoginVerify()
	else
		self:C2GSRegistAccount()
	end
end

--1.1客户端注册
function CLoginPhoneCtrl.C2GSRegistAccount(self)
	--已经联网 直接登录
	self.m_IsReconnect = false
	if(self.m_IsConnected) then
		netlogin.C2GSRegistAccount(self.m_VerifyInfo.account,self.m_VerifyInfo.pwd, nil)
	else
		local serverInfo = self.m_VerifyInfo.serverInfo
		if(not serverInfo) then
			printerror("服务器信息为空")
		end
		self:ConnnectServer(serverInfo.ip, serverInfo.ports, serverInfo)
	end
end

--账户注册成功
function CLoginPhoneCtrl.GS2CRegistAccount(self, mData)
	self:OnEvent(define.Login.Event.RegistAccountSuccess, mData)
end


--1.2客户端登录
function CLoginPhoneCtrl.C2GSLoginVerify(self, account, pwd)
	self.m_IsReconnect = false
	if(self.m_IsConnected ) then
		netlogin.C2GSLoginVerify(self.m_VerifyInfo.account,self.m_VerifyInfo.pwd)
	else
		local serverInfo = self.m_VerifyInfo.serverInfo
		if(not serverInfo) then
			printerror("服务器信息为空")
...
function CLoginPhoneCtrl.GS2CLoginVerify(self, mData)
	self:OnEvent(define.Login.Event.LoginVerifySuccess, mData)
end


--2进入游戏
function CLoginPhoneCtrl.C2GSQueryLogin(self)
	--检查导表数据更新
...
function CLoginPhoneCtrl.OnDataUpdateFinished(self)
	...
	-- if self.m_IsPC then
		if self.m_VerifyInfo.ispidlogin then
			...
			netlogin.C2GSLoginAccount(self.m_VerifyInfo.account, self.m_VerifyInfo.pwd, nil, Utils.GetDeviceModel(), self:GetPlatform(), Utils.GetMac(), gameconfig.Version.AppVer, "", UnityEngine.SystemInfo.operatingSystem, version, Utils.GetDeviceUID())
		end
	-- else
