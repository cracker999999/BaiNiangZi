...
function CWarrior.GoBack(self, iSpeed)
	...
	print("CWarrior.GoBack self.m_OriginPos iSpeed is ",self.m_OriginPos.x, self.m_OriginPos.z, iSpeed)
end
...
function CWarrior.RunTo(self, endPos, iSpeed, endAngle, cb, runback)
	...
	print("curpos endpos is ", curpos.x, curpos.z, endPos.x, endPos.z)
	if g_WarCtrl.g_Print or true then
		print(string.format("<color=#00FFFF> >>> .%s | %s </color>", "RunTo", "开始移动位置"), self:GetName() .. self.m_ID, endPos.x,endPos.z, iSpeed, endAngle, cb, runback, dis)
		print("调用堆栈:\n",debug.traceback())
	end
