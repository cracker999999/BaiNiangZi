...
function CJjcSingleBox.OnClickChallenge(self)
	...
	if self.m_Tag == "single" then
		if g_JjcCtrl.m_LeftCount <= 0 then
			if g_JjcCtrl.m_JjcMainHasBuy >= data.jjcdata.BUYJJCTIME[#data.jjcdata.BUYJJCTIME].hasbuy[2] then
				...
				local windowConfirmInfo = {
					msg				= string.gsub(data.jjcdata.TEXT[define.Jjc.Text.AddCountBtn].content, "#num", self:GetJJCBuyTimeMoney(g_JjcCtrl.m_JjcMainHasBuy + 1).."仙玉"),
					title			= "提示",
					okCallback = function ()
		...
		if g_JjcCtrl.m_JjcMainCountTime and g_JjcCtrl.m_JjcMainCountTime > 0 then
			local windowConfirmInfo = {
				msg				= string.gsub(data.jjcdata.TEXT[define.Jjc.Text.SpeedTimeBtn].content, "#num", math.ceil(tonumber(Value)).."仙玉"),
				title			= "提示",
				okCallback = function ()
...
function CJjcSingleBox.OnClickAddCount(self)
	if g_JjcCtrl.m_JjcMainHasBuy >= data.jjcdata.BUYJJCTIME[#data.jjcdata.BUYJJCTIME].hasbuy[2] then
		...
		local windowConfirmInfo = {
			msg				= string.gsub(data.jjcdata.TEXT[define.Jjc.Text.AddCount].content, "#num", self:GetJJCBuyTimeMoney(g_JjcCtrl.m_JjcMainHasBuy + 1).."仙玉"),
			title			= "提示",
			okCallback = function ()
				netjjc.C2GSJJCBuyFightTimes()
			end,
...
function CJjcSingleBox.OnClickSpeedTime(self)
	if g_JjcCtrl.m_JjcMainCountTime > 0 then
		...
		local Value = load(string.format([[return (%s)]], ValueStr))()
		local windowConfirmInfo = {
			msg				= string.gsub(data.jjcdata.TEXT[define.Jjc.Text.SpeedTime].content, "#num", math.ceil(tonumber(Value)).."仙玉"),
			title			= "提示",
			okCallback = function ()
				netjjc.C2GSJJCClearCD()
			end,
