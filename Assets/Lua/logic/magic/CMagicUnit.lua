...
define.MagicUnit = {
	-- 需要特殊判断的,并只有一段的技能列表标记(暂时没用的)
	MagicIDList = {1201, 7701, 7801}
}

function CMagicUnit.ctor(self, id, shape)
	self.m_ID = id
	self.m_Shape = shape
	self.m_MagicID = 1
	self.m_MagicIdx = 1
...
function CMagicUnit.Start(self, oCmd)
	...
	if g_WarCtrl.g_Print then
		printc(string.format("CMagicUnit.Start 法术开始 | magic:%s | shape:%s | index | mID:%s", self.m_MagicID, self.m_Shape, self.m_MagicIdx, self.m_ID))
	end
...
function CMagicUnit.BuildCmds(self, dFile)
	print("CMagicUnit.BuildCmds 调用堆栈:\n",debug.traceback())
	self.m_CmdList = {}
	for i, dCmdData in ipairs(dFile.cmds) do
		local oCmd = CMagicCmd.New(dCmdData.func_name, dCmdData.start_time, dCmdData.args, self)
		...
		if lGroupCmds then
			...
			for _, dCmdData in ipairs(lGroupCmds) do
				if dCmdData.func_name == "GroupTime" then
					...
					local oCmd = CMagicCmd.New(dCmdData.func_name, iStartTime+iDuration+dCmdData.start_time, dCmdData.args, self)
...
function CMagicUnit.AddHitInfo(self, iHitTime, iHurtDelta, bFaceAtk, bAnim, bDamageFollow, bConsiderHight, bShot)
	...
	local dInfo = {
		...
		hit_shot = bShot == nil and true or bShot,
	}
...
function CMagicUnit.GetDesc(self)
	return string.format("Magic:%s_%s_%s | ID:%s", self.m_MagicID, self.m_Shape, self.m_MagicIdx, self.m_ID)
end

function CMagicUnit.IsGarbage(self)
	if self.m_MagicIdx > 1 and #self.m_CmdList == 1 then
		local dMagicTimeInfo = DataTools.GetMagicTimeInfo(self.m_MagicID, self.m_Shape, self.m_MagicIdx)
		if dMagicTimeInfo and self.m_MagicIdx == #dMagicTimeInfo then
			self.m_CmdList[1]:ClearCmd()
		end
	end
