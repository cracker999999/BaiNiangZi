...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		{	
            ...
            //拷贝mods下对应sdk的目录到XCode工程下的demiSdkFolder
            string demiSdkFolder = this.projectRootPath + "/" + "demiSdkFolder";
            if (!Directory.Exists(demiSdkFolder))
            {
                Directory.CreateDirectory(demiSdkFolder);
            }
            //拷贝过去
            this.CopyDir(mod.path + "/iOS/" + mod.name, demiSdkFolder + "/iOS/" + mod.name);
			...
			foreach( string filePath in mod.files ) {
                ...
                string absoluteFilePath = System.IO.Path.Combine(demiSdkFolder, fPath);
			...
			if (mod.embed_binaries != null)
			{
				...
				foreach( string binary in mod.embed_binaries ) {
					string absoluteFilePath = System.IO.Path.Combine(demiSdkFolder, binary );
			...
			foreach( string folderPath in mod.folders ) {
                ...
                string absoluteFolderPath = System.IO.Path.Combine(demiSdkFolder, fPath);
	    ...
	    public void CopyDir(string fromDir, string toDir)
	    {
	        ...
	        foreach (var fromFileName in files)
	        {
	            string fileName = Path.GetFileName(fromFileName);
	            if (string.IsNullOrEmpty(fileName))
	            {
	                Debug.LogError("错误，文件名解析错误,file="+ fromFileName);
	                continue;
	            }
            ...
            foreach(string fromDirName in dirs)
            {
                string dirName = Path.GetFileName(fromDirName);
                string toDirName = Path.Combine(toDir, dirName);
                if (string.IsNullOrEmpty(dirName))
                {
                    Debug.LogError("错误，文件名解析错误,file=" + fromDirName);
                    continue;
                }
