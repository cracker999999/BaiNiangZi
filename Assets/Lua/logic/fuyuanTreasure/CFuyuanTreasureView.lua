...
function CFuyuanTreasureView.GetRewardPos(self, i)--奖品位置
    -- if i > 12  then
    --     return Vector3.New(-600, 400, 0)
    -- end
...
function CFuyuanTreasureView.RefreshRewardList(self)
    ...
    for k , v in ipairs( self.m_rewardList) do
        ...
        box:SetData(v)
        box:SetLocalPos(self:GetRewardPos(k))
        --printc(box:GetLocalPos())
    end
...
function CFuyuanTreasureView.ChooseItemAni(self, endIndex, endCb)
    ...
    local function onUpdate(value)
        ...
        local box = self.m_BoxItemList[idx]
        DOTween.DOLocalMove(self.m_TurnNode.m_Transform, box:GetLocalPos(), 0.01)
        --printc(idx, box:GetLocalPos())
    end

    DOTween.DoValue(1, endIndex, 2, onUpdate, onEnd)
