...
function CScheduleCrapsView.OnSure(self)
    ...
    --每天首次花费仙玉摇骰子时(即剩余次数为10时)，弹出二次确认
    if self.m_LastCount == self.m_MaxGoldCount then
        local args = {
            msg = "免费次数已用完，你确定花仙玉摇骰子吗？",
            title = "提示",
            okCallback = function ()
