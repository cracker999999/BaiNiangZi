...
function CQuickGetCoinView.SetInfo(self, info)
	...
	self.m_TipsLab:SetText("是否使用".. tostring(goldcoin) .."仙玉代替(额外兑换的".. tostring(exchangemoneyvalue - moneyvalue) .. itemdata.name .."会自动返还)")
	self.m_ExchangeLab:SetText("使用仙玉#cur_2".. tostring(goldcoin))
...
function CQuickGetCoinView.InitCoinInfo(self, itemlist)
	...
	elseif self.m_CoinData.sid == 1003 then --仙玉
		str = "1"
	elseif self.m_CoinData.sid == 1004 then --绑定仙玉
		str = "1"
	end
	...
	if self.m_CoinData.sid~=1003 then
		self.m_TipsLab:SetText("是否使用"..quotient.."仙玉代替(额外兑换的"..remainder..itemdata.name.."会自动返还)")
		if g_AttrCtrl.goldcoin + g_AttrCtrl.rplgoldcoin >=  self.m_CostCash then
			self.m_ExchangeLab:SetText("使用仙玉#cur_2"..self.m_CostCash)
		else
			self.m_ExchangeLab:SetText("使用仙玉#cur_2#R"..self.m_CostCash.."#n")
		end
	else
		self:QuickGetCB()
		self:CloseView()
