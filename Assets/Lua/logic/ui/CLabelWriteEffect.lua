...
function CLabelWriteEffect.InitMskIdx(self, text)
	...
	for i = 1, #self.m_Text do
		...
		if self.m_Text[i] == "#" and self.m_Text[i + 1] then
			...
			if datauser.colordata.COLORINDARK_2[str] then
				self.m_MaskIdx[i] = datauser.colordata.COLORINDARK_2[str]
				self.m_MaskIdx[i + 1] = datauser.colordata.COLORINDARK_2[str]
