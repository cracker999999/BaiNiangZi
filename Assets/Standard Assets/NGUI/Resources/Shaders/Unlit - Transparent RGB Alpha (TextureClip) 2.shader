...
Shader "Hidden/Unlit/Transparent RGB Alpha (TextureClip) 2"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (v2f IN) : COLOR
			{
				...
				// Sample the texture
				half4 col = half4(tex2D(_MainTex, IN.texcoord).rgb, tex2D(_AlphaTex, IN.texcoord).r);
				if(IN.gray == 0)
				{
				...
				half clipCol = tex2D(_ClipTex, IN.clipUV).a; 
				col.a *= clamp(f, 0.0, 1.0) * clipCol;
				return col;
			}
			ENDCG
		}
