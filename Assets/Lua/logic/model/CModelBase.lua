...
function CModelBase.Ranse(self, matRansePath, ranseInfo, cb)
	self:SetWasColor(ranseInfo)
	if not next(ranseInfo) then return end
...
function CModelBase:SetWasColor(ranseInfo)
	local coms = self.m_GameObject:GetComponentsInChildren(typeof(C_api.WasImage),true)
	if not coms then return end
	for i = 0, coms.Length-1 do
		local com = coms[i]
		if ranseInfo and next(ranseInfo) then
			--printerror("==============================================ranseInfo="..table_tostring(ranseInfo))
			--PartType = { hair = 1, clothes = 2, other = 3, pant = 4, },
			local colors = {}
			if ranseInfo[define.Ranse.PartType.hair] then 	
				colors[define.Ranse.PartType.hair] = ranseInfo[define.Ranse.PartType.hair];
			else 
				colors[define.Ranse.PartType.hair] = Color.New(0,0,0,0)
			end 

			if ranseInfo[define.Ranse.PartType.clothes] then 	
				colors[define.Ranse.PartType.clothes] = ranseInfo[define.Ranse.PartType.clothes];
			else 
				colors[define.Ranse.PartType.clothes] = Color.New(0,0,0,0)
			end 
			if ranseInfo[define.Ranse.PartType.other] then 	
				colors[define.Ranse.PartType.other] = ranseInfo[define.Ranse.PartType.other];
			else 
				colors[define.Ranse.PartType.other] = Color.New(0,0,0,0)
			end 
			if ranseInfo[define.Ranse.PartType.pant] then 	
				colors[define.Ranse.PartType.pant] = ranseInfo[define.Ranse.PartType.pant];
			else 
				colors[define.Ranse.PartType.pant] = Color.New(0,0,0,0)
			end 
			com:setColor(define.Ranse.PartType.clothes,  colors)
		else
			com:clean()
		end 
	end
