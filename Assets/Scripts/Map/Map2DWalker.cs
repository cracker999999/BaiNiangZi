...
{
    public Player2DAnimator[] player;
    public Pet2DAnimator[] pet;
    ...
    private int[] dir = new[] { 6, 2, 5, 1, 4, 0, 7, 3 };
    ...
    public void Update()
    {
        ...
        {
            player = GetComponentsInChildren<Player2DAnimator>();//leen+
            if (player != null && player.Length > 0)
            {
                player.ForEach((csd) =>
                {
                    csd.RefreshFrames("stand", dir[GetDir(desPos)]);
                    if(csd.weapon)
                        csd.weapon.RefreshFrames("stand", csd.weapon.站立动画高度);
                });
            }
        }
    ...
    private void ClearPath()
    {
        ...
        isPathing = false;
        linePath.Clear();
        pet = GetComponentsInChildren<Pet2DAnimator>();
        if (pet.Length > 0)
            pet.ForEach((csd) =>
            {
                csd.RefreshFrames("stand", 0);
            });
    }
    ...
    private void Translate(Vector3 pos)
    {
        ...
        if (pet.Length > 0)
            pet.ForEach((csd) =>
            {
                csd.RefreshFrames("walk", GetDirPet(pos));
            });

        player = GetComponentsInChildren<Player2DAnimator>();
        if (player.Length > 0)
            player.ForEach((csd) =>
            {
                if (csd.IsRide())
                {
                    csd.RefreshFrames("runRide", dir[GetDir(pos)]);
                    if(csd.weapon)
                        csd.weapon.RefreshFrames("stand", csd.weapon.站立动画高度);
                }
                else
                {
                    csd.RefreshFrames("walk", dir[GetDir(pos)]);
                    if (csd.weapon)
                        csd.weapon.RefreshFrames("walk", csd.weapon.行走动画高度);
                }
            });

        var wing = GetComponentsInChildren<Wing2DAnimator>();
        if (wing.Length > 0)
            wing.ForEach((csd) =>
            {
                csd.direction =  dir[GetDir(pos)];
            });
    }

    public int GetDir(Vector3 pos)
    {
        var angle = Vector3.Angle(pos, Vector3.up);
        var isRight = Vector3.Cross(pos, Vector3.up).z > 0;
        if (isRight)
            angle = 360 - angle;
        if (angle > 337.5f)
            angle -= 360;
        return Mathf.FloorToInt((angle + 22.5f) / 45);
    }

    public int GetDirPet(Vector3 pos)
    {
        var angle = Vector3.Angle(pos, Vector3.up);
        var isRight = Vector3.Cross(pos, Vector3.up).z > 0;
        if (isRight)
