...
function CMainMenuRB.ctor(self, obj)
	...
	self.m_JjcBtn = self:NewUI(15, CButton)
	...
	self.m_HorseBtn = self.m_HBtnFirstGrid:GetChild(5)

	self.m_BadgeBtn = self.m_TempGrid:GetChild(4)
...
function CMainMenuRB.InitContent(self)
	...
	self.m_Timer = Utils.AddTimer(callback(self, "Update"), 1, 0)
end

function CMainMenuRB.Update(self)

	if UnityEngine.Input.GetKeyDown(enum.KeyCode.LeftAlt) then
		self.isAlt = true
	end
	if UnityEngine.Input.GetKeyUp(enum.KeyCode.LeftAlt) then
		self.isAlt = false
	end
     -- 设置按钮快捷键
	if self.isAlt and UnityEngine.Input.GetKeyDown(enum.KeyCode.B) then
		if CItemMainView:GetView() then
			CItemMainView:CloseView()
		else
			self:OnItem() --背包  以弄好 不需要在动  
		end
	end
	self.m_Timer = Utils.AddTimer(callback(self, "Update"), 1, 0)
end
