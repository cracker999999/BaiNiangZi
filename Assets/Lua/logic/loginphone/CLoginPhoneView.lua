...
function CLoginPhoneView.OnCreateView(self)
	...
	self.m_RegistBox = self:NewUI(18, CBox)

	--PC上的逻辑
	self:PCAccountContent()
	self:InitContent()
	self:InitTexture()
...
function CLoginPhoneView.PCAccountContent(self)
	--登陆
	...
	self.m_LoginAccountBtn:AddUIEvent("click", callback(self, "OnClickLoginAccountBtn"))
	self.m_RegistLabel = self.m_AccountBox:NewUI(5, CLabel)
	self.m_RegistLabel:AddUIEvent("click", callback(self,"OnClickRegistLabel"))
	...
	if not gameconfig.Model.YunceModel and verify then
		if(UnityEngine.PlayerPrefs.GetString(verify.account, "") == "") then
			UnityEngine.PlayerPrefs.SetString(verify.account, verify.pwd)
		end
	...
	--注册
	self.m_RegistNameInput = self.m_RegistBox:NewUI(1, CInput)
	self.m_RegistPwdInput = self.m_RegistBox:NewUI(2, CInput)
	self.m_RegistRePwdInput = self.m_RegistBox:NewUI(3, CInput)
	self.m_RegistAccountBtn = self.m_RegistBox:NewUI(4, CButton)
	self.m_RegistBackBtn = self.m_RegistBox:NewUI(5, CButton)
	self.m_RegistAccountBtn:AddUIEvent("click", callback(self, "OnClickRegistAccountBtn"))
	self.m_RegistBackBtn:AddUIEvent("click", callback(self, "OnClickRegistBackBtn"))
end
...
function CLoginPhoneView.InitContent(self)
	...
	g_LoginPhoneCtrl:AddCtrlEvent(self:GetInstanceID(), callback(self, "OnCtrlLoginEvent"))
end
...
function CLoginPhoneView.OnCtrlLoginEvent(self, oCtrl)
	if oCtrl.m_EventID == define.Login.Event.RegistAccountSuccess then
		local data = oCtrl.m_EventData
		g_NotifyCtrl:FloatMsg("注册成功！")
		self.m_AccountBox:SetActive(true)
		self.m_RegistBox:SetActive(false)
		self.m_AccountInput:SetText(data.account)
		self.m_PwdInput:SetText(data.pwd)
	elseif oCtrl.m_EventID == define.Login.Event.LoginVerifySuccess then
		g_NotifyCtrl:FloatMsg("登录成功")
		--显示用户公告和协议
...
function CLoginPhoneView.OnClickLoginAccountBtn(self)
	...
	if not t.ispidlogin then
		...
		if #t.account < self.m_MinInputChar then
			g_NotifyCtrl:FloatMsg("用户名长度小于" .. self.m_MinInputChar)
			return
		end
		if not t.pwd or t.pwd == "" then
			g_NotifyCtrl:FloatMsg("请输入密码")
			return
		end
		if #t.pwd < self.m_MinInputChar then
			g_NotifyCtrl:FloatMsg("密码长度小于" .. self.m_MinInputChar)
			return
		end
		-- local verifyPwd = UnityEngine.PlayerPrefs.GetString(t.account, "")
		-- if(verifyPwd == "") then
		-- 	g_NotifyCtrl:FloatMsg("没有当前用户，请先注册")
		-- 	return
		-- end
		-- if(verifyPwd ~= t.pwd) then
		-- 	g_NotifyCtrl:FloatMsg("密码错误")
		-- 	return
		-- end
	else
		if not t.account or t.account == "" then
	...
	g_LoginPhoneCtrl:C2GSLoginVerify()
end

function CLoginPhoneView.OnClickRegistLabel(self)
	self.m_AccountBox:SetActive(false)
	self.m_RegistBox:SetActive(true)
end

function CLoginPhoneView.OnClickRegistAccountBtn(self)
	local account = self.m_RegistNameInput:GetText()
	local pwd1 = self.m_RegistPwdInput:GetText()
	local pwd2 = self.m_RegistRePwdInput:GetText()
	if(not account or account == "") then
		g_NotifyCtrl:FloatMsg("请输入用户名")
		return
	end
	if(not pwd1 or pwd1 == "") then
		g_NotifyCtrl:FloatMsg("请输入密码")
		return
	end
	if(not pwd2 or pwd2 == "") then
		g_NotifyCtrl:FloatMsg("请输入确认密码")
		return
	end
	if(account == pwd1) then
		g_NotifyCtrl:FloatMsg("账号和密码不能相同")
		return
	end
	if(pwd1 ~= pwd2) then
		g_NotifyCtrl:FloatMsg("密码输入不一致")
		return
	end
	--走服务端注册流程
	local t = {
	...
	g_LoginPhoneCtrl:C2GSRegistAccount()
	-- else
	...
	-- if(UnityEngine.PlayerPrefs.GetString(account, "") ~= "") then
	-- 	g_NotifyCtrl:FloatMsg("账号已经被注册")
	-- 	return
	-- end
	-- UnityEngine.PlayerPrefs.SetString(account, pwd1)
	-- g_NotifyCtrl:FloatMsg("注册成功！")
	-- self.m_AccountBox:SetActive(true)
	-- self.m_RegistBox:SetActive(false)
	-- self.m_AccountInput:SetText(account)
	-- self.m_PwdInput:SetText(pwd1)
end

function CLoginPhoneView.OnClickRegistBackBtn(self)
	self.m_AccountBox:SetActive(true)
	self.m_RegistBox:SetActive(false)
end
...
function CLoginPhoneView.GetServer(self)
...
function CLoginPhoneView.OnClickLoginGame(self)
	...
	g_LoginPhoneCtrl:C2GSQueryLogin()
	-- if g_LoginPhoneCtrl.m_IsPC then
