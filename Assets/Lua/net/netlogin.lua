...
function GS2CLoginError(pbdata)
	...
	elseif errcode == 1020 then
		...
		g_LoginPhoneCtrl:ResetConnected()
	...
	elseif errcode == 1022 then
		printerror("账号已经注册")
		g_NotifyCtrl:FloatMsg("账号已经注册")
		g_LoginPhoneCtrl:ResetConnected()
	end
	print("Login err", pid, errcode)
end

function GS2CRegistAccount(pbdata)
	g_LoginPhoneCtrl:GS2CRegistAccount(pbdata)
end

function GS2CLoginVerify(pbdata)
	g_LoginPhoneCtrl:GS2CLoginVerify(pbdata)
end
...
--C2GS--
function C2GSRegistAccount(account, pwd, app_ver)
	local t = {
		account = account,
		pwd = pwd,
		app_ver = app_ver,
	}
	g_NetCtrl:Send("login", "C2GSRegistAccount", t)
end

function C2GSLoginVerify(account, pwd)
	local t = {
		account = account,
		pwd = pwd,
	}
	g_NetCtrl:Send("login", "C2GSLoginVerify", t)
end

function C2GSLoginAccount(account, pwd, token, device, platform, mac, app_ver, imei, os, client_ver, udid)
	local t = {
		account = account,
		pwd = pwd,
		token = token,
