...
function CGmCtrl.SetRecord(self, sParam)
	if sParam then
		...
		if self.m_Record.instruct then
			...
			for k,v in ipairs(self.m_Record.instruct) do
				if v.name == sParam then
					table.remove(self.m_Record.instruct, k)
				end
			end
			...
			if updateIndex > 0 and not self.m_GMRecord.Logic.recordSame then
				table.remove(self.m_Record.instruct, updateIndex)
			else
				if #self.m_Record.instruct == 20 then
